...
static void DeleteScriptEnvironment(IScriptEnvironment* env) {
	if(env) env->DeleteScriptEnvironment();
}

static std::unique_ptr<IScriptEnvironment, decltype(&DeleteScriptEnvironment)> make_unique_ptr(IScriptEnvironment* env) {
	return std::unique_ptr<IScriptEnvironment, decltype(&DeleteScriptEnvironment)>(env, DeleteScriptEnvironment);
}
