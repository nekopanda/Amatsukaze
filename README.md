...
※**(重要)x265でエンコードする場合、インタレ解除が必須なので、NVIDIA GPU搭載PC推奨です。CUDAが使えないと残念な速度になります！**
（Compute Capability 3.5以上と、比較的新しいそれなりのGPUが必要です。GTX1060とか）
...
<img src="https://i.imgur.com/zATaFe4.png" width="705">

出力先選択パネルが出るので、「出力先」を設定して、「テスト」または「通常」を選択。
...
「エンコード設定」タブで「エンコーダ追加オプション」を入力します。

<img src="https://i.imgur.com/xPPNErg.png" width="520">

CRF22にしてみました。プロファイルの設定を変更したら、「適用」ボタンを押します。
「適用」ボタンを押さないと、設定は反映されないし、保存もされないので注意してください。
...
プロファイルは、自動選択と通常のプロファイルの2種類があります。自動選択は、プロファイル選択で `自動選択_` で始まる項目で、これは、ファイルによって使うプロファイルを分けたい場合に使います。初期状態だと、「自動選択_デフォルト」がありますが、これはプロファイルを何も選択しない設定になっているので、これを選んでも、プロファイルが選択されません。最初は「デフォルト」を選択してください。

<img src="https://i.imgur.com/nwnPnA7.png" width="374">
...
「エンコード設定」タブに戻って、エンコーダを「QSVEnc」、追加オプションを入力します。「適用」ボタンも忘れずに。

<img src="https://i.imgur.com/q6i0MWQ.png" width="515">
...
<img src="https://i.imgur.com/82AVjj1.png" width="520">

#### 4-2. 「キュー」タブにTSファイルをドラッグ&ドロップ

この状態でTSファイルを投入すると、最初はロゴファイルがないので、エンコードが始まりません。

<img src="https://i.imgur.com/VUZDZYt.png" width="520.6">
...
<img src="https://i.imgur.com/TSWCoQ7.png" width="520.6">
...
<img src="https://i.imgur.com/itVqfk8.png" width="507.3">
